...
This report documents the serial execution of the multi-container project. Each part includes the commands used and a short explanation.
...
0.0.0.0: Binds to all available network interfaces, making the Rishabhmeta app accessible to any device on the network.

🏗️ PART 5 - Production Best Practices
1. Image Pinning 📌
Practice: Used node:18-alpine and postgres:15 instead of latest.
Explanation: Pinned specific image versions to ensure build consistency. The latest tag is avoided because it is non-deterministic and can lead to unexpected breaking changes.



2. Secret Management 🔑
Practice: Moved credentials to .env.
Explanation: Secured sensitive data like database passwords by using environment variables, ensuring no secrets are hardcoded in the source code.

📊 PART 6 - Monitoring
1. Resource Statistics 🖥️
Command:

Bash
docker stats --no-stream
Explanation: Monitored real-time CPU and Memory usage of all Rishabhmeta containers to ensure the system is operating within healthy limits.



2. OOM (Out-of-Memory) Logic 💥
Explanation: If a container exceeds host memory limits, the Linux Kernel's OOM Killer will terminate the process. Monitoring prevents such unexpected shutdowns.
